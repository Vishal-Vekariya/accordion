...
class Accordion {
  constructor(domNode) {
    this.accordion = domNode;
     // Fix: Grab all buttons with the 'accordion' class from within the accordion container
  ...
  togglePanel(button) {
    // Fix: Use ARIA attributes to toggle the expanded state for better accessibility
    const isExpanded = button.getAttribute('aria-expanded') === 'true';
    // Fix: Use the 'aria-controls' attribute to find the corresponding content panel
    ...
    if (!isExpanded) {
      // Fix: Set max-height to the scrollHeight to animate the panel opening
      content.style.maxHeight = content.scrollHeight + "px";
    } else {
      // Fix: Reset max-height to null to collapse the panel
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Fix: Create an instance of the Accordion class for better modularity and code reuse
